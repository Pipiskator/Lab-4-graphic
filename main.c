...
#define W 10 // Ширина карты
#define H 10 // Высота карты
...
void Check_Tile_Collision(Hero *hero) {
    ...
    if (hero->dx > 0) { // Движение вправо
        for (int i = tileY; i < tileY + objH; i++) {
            ...
            if (tileX + objW < 10 && TileMap[i][tileX + objW] == 'B') {
    ...
    if (hero->dy > 0) { // Движение вниз
        for (int j = tileX; j < tileX + objW; j++) {
            ...
            if (tileY + objH < 10 && TileMap[tileY + objH][j] == 'B') {
    ...
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            if (TileMap[i][j] == 'B') {
                ...
                glColor3f(0.0f, 0.8f, 0.5f); // Серый цвет для блоков
                glBegin(GL_QUADS);
                glVertex2f(x, y);
