...
void ShowButton(int buttonId)
{
    ...
    if(btn1.isDown)
        glColor3f(0.0, 1.0, 0.0); // Цвет для нажатой кнопки (зеленый)
    else if(btn1.isHover)
        glColor3f(0.5, 0.5, 1.0); // Цвет для кнопки, на которую наведен курсор (светло-синий)
    else
        glColor3f(0.8, 0.8, 0.8); // Цвет для обычной кнопки (серый)
